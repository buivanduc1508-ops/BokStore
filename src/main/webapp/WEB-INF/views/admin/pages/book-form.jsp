...
<div class="bok-admin">
  <header>
    <div>
      ...
      <h1>
        <%=b==null?"Thêm sản phẩm":"Chỉnh sửa sản phẩm"%>
      </h1>
    </div>
    <a href="<%=request.getContextPath()%>/admin/manage?tab=books">
      Quay lại
    </a>
  </header>
  <section class="admin-form-card">
    <form class="form-grid" method="post" action="<%=request.getContextPath()%>/admin/manage">
      ...
      <label>
        Tên sách
        <input name="name" required value="<%=b==null?"":b.getName()%>">
      </label>
      <label>
        Tác giả
        <input name="author" required value="<%=b==null?"":b.getAuthor()%>">
      </label>
      <label>
        Nhà xuất bản
      ...
      <label>
        Tồn kho
        <input name="stock" type="number" min="0" required value="<%=b==null?0:b.getStock()%>">
      </label>
      <label>
        Danh mục
    ...
    <%if(b!=null){%>
      ...
      <form class="inline-form" method="post" action="<%=request.getContextPath()%>/admin/manage">
        ...
        <input name="quantity" type="number" min="1" required placeholder="Số lượng nhập thêm">
        <button>
          Nhập thêm kho
        </button>
      </form>
